...
class SearchEnhancedKnowledgeDrafter(BaseAgent):
    ...
    def draft(self, payload: KnowledgeDraftPayload | Mapping[str, Any] | str):
        ...
        if isinstance(raw_output, str):
            try:
                raw_output = convert_json_output(raw_output)
            except Exception:
                try:
                    ...
                    try:
                        import ast as _ast
                        raw_output = _ast.literal_eval(raw_output)
                    except Exception:
                        raw_output = {}
...
def draft_knowledge_points_with_llm(
    ...
    *,
    ...
    # Note: parsing is now handled in main.py, so these are already Python objects
